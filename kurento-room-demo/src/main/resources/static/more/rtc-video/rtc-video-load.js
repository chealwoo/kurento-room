...
function loadAddOns() {
	...
	if (inQ.initialized !== true) {
	  top.inQ.initialized = true;
	  top.inQ.addOns = {};
	  top.inQ.loadedAddOns = [];
	  top.inQ.requirejQuery = function() {top.inQ.jQueryRequired = true;};
	  top.inQ.loadScript = function(url) {
		var regex = new RegExp(url, 'gi');
		if (top.inQ.loadedAddOns.join().match(regex)) {return;}
		top.inQ.loadedAddOns.push(url);
		if (url.endsWith(".js")) {
	  ...
	  top.inQ.debugMode = location.href.match(/tcDebugMode=true/i) !== null;
	  top.inQ.console = function() {if (top.inQ.debugMode && window.console) window.console.debug(arguments);};
	  top.inQ.loadScript('https://mediav3.inq.com/media/sites/320/flash/SolutionsAssets/br3-addons/safeJQuery.js');
	}

	for (var i = 0; i < arguments.length; i++) {top.inQ.loadScript(arguments[i]);}
}